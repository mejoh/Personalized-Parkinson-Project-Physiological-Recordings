...
% Input Arguments:
%
%   X - numObs-by-numVars matrix or dataset array of numObs observations on
%       numVars variables.
%
...
% $Revision: 1.1.6.2 $ $Date: 2011/11/09 16:44:24 $
...
parseObj.addParamValue('rows','all',@rowsCheck);
...
if isempty(varNames)
    ...
    if isa(X,'dataset')
        ...
        varNames = X.Properties.VarNames;
...
varNames = cellfun(@(s)s(1:25),varNames,'UniformOutput',false);
...
%         axis normal   
        ...
        if i ~= j
            ...
            annotation('textbox',plotPos,...
                       'String',['R = ' num2str(R(i,j),'%3.2f') ' || p = ' num2str(PValue(i,j),'%3.2f')],...
                       'FontWeight','Bold',...
                       'Color',corrColor,...
...
try
    ...
    X = double(X); % Handle dataset arrays
