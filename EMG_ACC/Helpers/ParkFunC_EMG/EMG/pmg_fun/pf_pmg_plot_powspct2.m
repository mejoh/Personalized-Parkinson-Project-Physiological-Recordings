function pf_pmg_plot_powspct2(conf,freqana,uSub)
...
%                     disp('probably needs debugging'); keyboard;
                ...
                iHand        =   pf_strcmp(conf.fa.fig.chan(:,1),CurHand);
                ...
                colo    =   eval([conf.fa.fig.col '(colocnt)']);
...
%             keyboard
            if strcmp(conf.fa.fig.pow.peaksel.onoff,'on') && ~strcmp(conf.fa.fig.pow.graph,'contourf')
               is      = pf_pmg_seltremorUI(hh,tits,chans,figPS,figP,figF,CurHand,conf.ft.chans(:,2),conf.fa.fig.pow.peaksel.peakdef,conf.fa.fig.pow.peaksel.mancheck);
               is      = [repmat(str2double(CurSub(2:end)),size(is,1),1) is]; % Add subject number
